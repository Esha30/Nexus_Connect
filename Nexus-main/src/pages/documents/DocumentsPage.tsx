        ...
        {doc.category === 'pitch_deck' && (
          <div className="flex gap-2">
            <Button
              ...
              title={doc.aiAnalysis?.summary ? 'View AI Analysis' : 'Run AI Pitch Analysis'}
              onClick={() => setShowAnalysis(showAnalysis === doc._id ? null : doc._id)}
            >
              <Zap size={20} className={isAnalyzing === doc._id ? 'animate-pulse text-yellow-500' : ''} />
            </Button>
      ...
      {showAnalysis === doc._id && (
        <div className="px-8 pb-8 animate-in slide-in-from-top-2 duration-300">
          <div className="bg-gray-900 rounded-3xl p-8 border border-white/10 shadow-2xl relative overflow-hidden">
            ...
            <div className="flex justify-between items-start mb-8 relative z-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-gray-400 text-[10px] font-black uppercase tracking-widest">Powered by Gemini 2.5 Flash</p>
                </div>
              </div>
            ...
            {/* Loading State */}
            {isAnalyzing === doc._id ? (
              <div className="flex flex-col items-center justify-center py-20 relative z-10 gap-6">
                <div className="relative">
                  <div className="w-16 h-16 rounded-full border-4 border-primary-600/20 border-t-primary-600 animate-spin" />
                  <div className="absolute inset-0 flex items-center justify-center">
                    <Zap size={20} className="text-primary-400 animate-pulse" />
                  </div>
                </div>
                <div className="text-center">
                  <p className="text-white font-bold">Analyzing pitch deck...</p>
                  <p className="text-gray-400 text-xs mt-1">Gemini is reading your document and generating insights</p>
                </div>
              </div>

            ) : !doc.aiAnalysis?.summary ? (
              /* No Data State */
              <div className="flex flex-col items-center justify-center py-20 relative z-10 gap-4">
                <div className="w-16 h-16 rounded-2xl bg-white/5 flex items-center justify-center border border-white/10">
                  <Zap size={28} className="text-gray-500" />
                </div>
                <div className="text-center">
                  <p className="text-white font-bold">No analysis yet</p>
                  <p className="text-gray-400 text-sm mt-1">Run the AI engine to get a full SWOT analysis and investor readiness scores.</p>
                </div>
                <Button
                  onClick={() => handleAnalyze(doc._id)}
                  className="mt-2 bg-primary-600 hover:bg-primary-500 text-white rounded-2xl px-8 py-3 font-bold"
                >
                  <Zap size={16} className="mr-2" /> Start AI Analysis
                </Button>
              </div>
              ...
              <div className="relative z-10 space-y-8">
                ...
                <div className="bg-white/5 rounded-2xl p-6 border border-white/5">
                  ...
                  <p className="text-gray-200 text-sm leading-relaxed">{doc.aiAnalysis.summary}</p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                  {/* Left: Full SWOT - 4 quadrants */}
                  <div className="md:col-span-3 grid grid-cols-2 gap-4">
                    {/* Strengths */}
                    <div className="bg-emerald-950/30 rounded-2xl p-5 border border-emerald-500/10">
                      <h5 className="text-[10px] font-black text-emerald-400 uppercase tracking-widest flex items-center gap-2 mb-3">
                    ...
                    {/* Weaknesses */}
                    <div className="bg-red-950/30 rounded-2xl p-5 border border-red-500/10">
                      <h5 className="text-[10px] font-black text-red-400 uppercase tracking-widest flex items-center gap-2 mb-3">
                    ...
                    {/* Opportunities */}
                    <div className="bg-blue-950/30 rounded-2xl p-5 border border-blue-500/10">
                      <h5 className="text-[10px] font-black text-blue-400 uppercase tracking-widest flex items-center gap-2 mb-3">
                        <div className="w-1.5 h-1.5 bg-blue-500 rounded-full" /> Opportunities
                      </h5>
                      <ul className="space-y-2">
                        {doc.aiAnalysis.swot?.opportunities?.map((o, i) => (
                          <li key={i} className="text-xs text-gray-300 flex gap-2 leading-relaxed">
                    ...
                    {/* Threats */}
                    <div className="bg-orange-950/30 rounded-2xl p-5 border border-orange-500/10">
                      <h5 className="text-[10px] font-black text-orange-400 uppercase tracking-widest flex items-center gap-2 mb-3">
                        <div className="w-1.5 h-1.5 bg-orange-500 rounded-full" /> Threats
                      </h5>
                      <ul className="space-y-2">
                        {doc.aiAnalysis.swot?.threats?.map((t, i) => (
                          <li key={i} className="text-xs text-gray-300 flex gap-2 leading-relaxed">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="bg-white/5 rounded-2xl p-6 border border-white/5 flex flex-col items-center justify-center text-center">
                      ...
                      <p className="text-[10px] text-gray-600 mt-1">out of 10</p>
                    </div>

                    {/* Individual Scores */}
                    <div className="space-y-4">
                      {[
                        { label: 'Market', score: doc.aiAnalysis.scores?.market, color: 'bg-blue-500' },
                        { label: 'Product', score: doc.aiAnalysis.scores?.product, color: 'bg-emerald-500' },
                        { label: 'Team', score: doc.aiAnalysis.scores?.team, color: 'bg-purple-500' }
                      ].map((s, i) => (
                    ...
                    <button
                      onClick={() => handleAnalyze(doc._id)}
                      className="w-full py-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-gray-400 hover:text-white text-xs font-bold uppercase tracking-widest transition-all flex items-center justify-center gap-2"
                    >
