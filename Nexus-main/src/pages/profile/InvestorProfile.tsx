...
import { MessageCircle, MapPin, UserCircle, Briefcase, Calendar, BarChart3 } from 'lucide-react';
...
import { Edit3, Save, X, Plus, Trash2, Sparkles, CheckCircle2, AlertCircle } from 'lucide-react';
...
export const InvestorProfile: React.FC = () => {
 ...
 {investor.name.split(' ')[0]} <span className="text-primary-600">{investor.name.split(' ').slice(1).join(' ')}.</span>
 </h1>
 ...
 Senior Investor • <span className="text-gray-900 font-medium ml-1.5">{investor.profile?.totalInvestments || investor.totalInvestments || 0} Portfolios Managed</span>
 </p>
 ...
 <Badge key={index} variant="secondary" className="bg-primary-50 text-primary-600 border-none font-semibold text-xs px-3 py-1.5">
 {stage}
 </Badge>
 ))}
 </div>
 ...
 <Link to={`/meetings?recipientId=${investor.id || (investor as any)._id}&name=${encodeURIComponent(investor.name)}`}>
 <Button
 variant="outline"
 className="rounded-lg font-semibold border-gray-200 py-2.5 px-6"
 ...
 className="rounded-lg font-semibold py-2.5 px-5"
 ...
 {!isCurrentUser && !isEditing && (
    <Button
      variant="outline"
      className="rounded-lg font-semibold border-primary-200 text-primary-600 bg-primary-50 hover:bg-primary-100 py-2.5 px-5 shadow-sm overflow-hidden relative group"
    ...
    <Card className="border-primary-100 shadow-xl shadow-primary-500/5 bg-gradient-to-br from-white to-primary-50/30 rounded-2xl overflow-hidden animate-in slide-in-from-top-4 duration-500">
      <CardHeader className="bg-primary-600 text-white px-6 py-4 flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-lg font-bold tracking-tight">Nexus Synergy Matrix</h2>
        </div>
